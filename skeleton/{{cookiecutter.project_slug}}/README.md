...
Create a Slack app for running a development version of {{ cookiecutter.project_name }}:
...
5. Replace any occurences of `${botName}` with a name for your bot.
    - If you're developing in a shared workspace, consider a name like `YOUR_NAME-{{ cookiecutter.project_slug }}`.
...
of {{ cookiecutter.project_name }} with `docker-compose up`
